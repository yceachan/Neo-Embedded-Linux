...
虚拟地址空间 (Virtual Address Space, **VAS**) 是 Linux 给每个进程的"独占假象"：进程看到的指针是一套从 0 起的虚地址，**内核 + MMU + 页表**联手把它翻译成真实的物理地址。本篇用 [`demo/`](./demo/) 中的实测数据，把 VAS 的**布局结构、权限隔离、随机化、特殊区域**讲清楚，并指出每条线接下来通向哪一篇笔记。
...
| 内核 ELF 加载器 (`load_elf_binary`) | 按 LOAD 段的 `p_flags` 调 `mmap()` 把每段映射进 VAS，并设置正确的 `PROT_*` |
