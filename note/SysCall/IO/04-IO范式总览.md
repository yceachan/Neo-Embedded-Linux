...
进程注册 `SIGIO` 信号处理函数，当 fd 就绪时，**内核主动发送 `SIGIO` 信号**通知进程，进程在信号处理函数中读取数据。

```
App                        Kernel                   Device
 │                            │                        │
 ├─ 注册 SIGIO 处理函数        │                        │
 ├─ fcntl(F_SETSIG/F_SETFL)  │                        │
 │◄─────────────────────────────────────────────────────┤ (立即返回)
 │                            │                        │
 │         (进程继续运行)       │       数据就绪          │
 │                            │◄───────────────────────┤
...
| 优点 | 缺点 |
|------|------|
| 真正异步：主循环不受影响 | 信号处理函数中能调用的函数受限（async-signal-safe） |
| 无需轮询，CPU 利用率高 | 多个 fd 发 SIGIO，无法区分来源（需用 `F_SETSIG` + `siginfo`）|
| 适合低频事件 | 实时信号（SIGRTMIN+n）可解决但更复杂 |
...
真正意义上的异步：进程提交 IO 请求后**立即返回**，内核完成数据等待 **+** 数据拷贝，完成后通知进程（信号或回调）。

```
App                        Kernel
 │                            │
 ├─ aio_read(&cb) ───────────►│  (立即返回，不等待)
 │                            │
 │  (进程继续运行)             │  内核等待数据
 │                            │  内核完成 copy_to_user
 │                            │
...
`io_uring`（Linux 5.1+）是更高效的异步 IO 接口，通过共享内存环形队列实现**零拷贝提交**：
